...
class AdaptiveAugment:
  def __init__(self,
               apply_func=None,
               # Function representing the augmentation pipeline. Can be a fully-qualified name, a function object, or None.
               apply_args=None,  # Keyword arguments for the augmentation pipeline.
               ...
               tune_kimg=500,
               # Adjustment speed, measured in how many kimg it takes for the strength to increase/decrease by one unit.
               stat_decay_kimg=0,
               # Exponential moving average to use for training statistics, measured as the half-life in kimg. 0 = disable EMA.
               ):
  ...
  def init_validation_set(self, D_gpus, training_set):
    ...
    with tflib.absolute_name_scope('Validation'), tf.control_dependencies(None):
      with tf.device('/cpu:0'):
        self._valid_images_in = tf.compat.v1.placeholder(training_set.dtype, name='valid_images_in',
                                               shape=[None] + training_set.shape)
        self._valid_labels_in = tf.compat.v1.placeholder(training_set.label_dtype, name='valid_labels_in',
                                               shape=[None, training_set.label_size])
  ...
  def tune(self, nimg_delta):
    ...
    if self.tune_heuristic == 'rv':
      ...
      rv = (acc['Loss/scores/real'] - acc['Loss/scores/valid']) / max(
        acc['Loss/scores/real'] - acc['Loss/scores/fake'], 1e-8)
  ...
  def run_validation(self, minibatch_size):
    if self._valid_images is not None:
      ...
      tflib.run(self._valid_op, {self._valid_images_in: self._valid_images[indices],
                                 self._valid_labels_in: self._valid_labels[indices]})
...
def augment_pipeline(
    ...
    imgfilter_bands=None,  # Probability multipliers for individual frequency bands.
  ...
  if xint > 0:
    ...
    G_inv @= translate_2d_inv(tf.compat.v1.rint(t[:, 0] * width), tf.compat.v1.rint(t[:, 1] * height))
  ...
  if G_inv is not I_3:
    ...
    m_lo = np.ceil(tf.reduce_max(-cp, axis=[0, 2]) - [cx, cy] + Hz_pad * 2)
    m_hi = np.ceil(tf.reduce_max(cp, axis=[0, 2]) - [cx, cy] + Hz_pad * 2)
    ...
    images = tf.nn.depthwise_conv2d_backprop_input(input_sizes=shape, filter=Hz[np.newaxis, :], out_backprop=images,
    ...
    images = tfa.image.transform(images=images, transforms=transforms, output_shape=shape,
                                        interpolation='BILINEAR')

    # Downsample and crop.
    images = tf.nn.depthwise_conv2d(input=images, filter=Hz[np.newaxis, :], strides=[1, 1, 1, 1], padding='SAME',
                                    data_format='NHWC')
    images = tf.nn.depthwise_conv2d(input=images, filter=Hz[:, np.newaxis], strides=[1, 2, 2, 1], padding='SAME',
                                    data_format='NHWC')
  ...
  if imgfilter > 0:
    ...
    for i, band_strength in enumerate(imgfilter_bands):
      ...
      if debug_percentile is not None:
        t_i = 2 ** (tflib.erfinv(tf.broadcast_to(debug_percentile, [
          batch]) * 2 - 1) * imgfilter_std) if band_strength > 0 else tf.ones([batch])
