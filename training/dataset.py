  ...
  def get_minibatch_tf(self):
    ...
    if self.mirror_augment or self.mirror_augment_v:
      ...
      if self.mirror_augment:
        images = tf.where(tf.random.uniform([tf.shape(images)[0]]) < 0.5, images, tf.reverse(images, [3]))
      if self.mirror_augment_v:
        images = tf.where(tf.random.uniform([tf.shape(images)[0]]) < 0.5, images, tf.reverse(images, [2]))
      ...
      if self.label_size > 0:
        with tf.device('/cpu:0'):
          return tf.gather(self._tf_labels_var,
                           tf.random.uniform([minibatch_size], 0, self._np_labels.shape[0], dtype=tf.int32))
...
def load_dataset(path=None, use_raw=False, resolution=None, max_images=None, max_label_size=0, mirror_augment=False,
                 mirror_augment_v=False, repeat=True, shuffle=True, seed=None):
  _ = seed
