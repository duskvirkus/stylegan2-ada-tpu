...
def apply_bias_act(x, act='linear', gain=None, lrmul=1, clamp=None, bias_var='bias', trainable=True):
  b = tf.compat.v1.get_variable(bias_var, shape=[x.shape[1]], initializer=tf.initializers.zeros(), trainable=trainable) * lrmul
...
def dense_layer(x, fmaps, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
  ...
  w = get_weight([x.shape[1].value, fmaps], lrmul=lrmul, weight_var=weight_var, trainable=trainable,
...
def conv2d_layer(x, fmaps, kernel, up=False, down=False, resample_kernel=None, lrmul=1, trainable=True,
                 use_spectral_norm=False):
  w = get_weight([kernel, kernel, x.shape[1].value, fmaps], lrmul=lrmul, trainable=trainable,
                 use_spectral_norm=use_spectral_norm)
...
def modulated_conv2d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, lrmul=1,
                           fused_modconv=False, trainable=True, use_spectral_norm=False):
  assert not (up and down)
  assert kernel >= 1 and kernel % 2 == 1
  ...
  if x.dtype.name == 'float16' and not fused_modconv and demodulate:
    w *= np.sqrt(1 / np.prod(wshape[:-1])) / tf.reduce_max(tf.abs(w),
                                                           axis=[0, 1, 2])  # Pre-normalize to avoid float16 overflow.
  ww = w[np.newaxis]  # [BkkIO] Introduce minibatch dimension.

  # Modulate.
  s = dense_layer(y, fmaps=x.shape[1].value, weight_var='mod_weight', trainable=trainable,
                  use_spectral_norm=use_spectral_norm)  # [BI] Transform incoming W to style.
  ...
  # Demodulate.
  if demodulate:
    d = np.rsqrt(tf.reduce_sum(tf.square(ww), axis=[1, 2, 3]) + 1e-8)  # [BO] Scaling factor.
...
def normalize_2nd_moment(x, axis=1, eps=1e-8):
  return x * np.rsqrt(tf.reduce_mean(tf.square(x), axis=axis, keepdims=True) + eps)
...
def apply_spectral_norm(w, state_var='sn', iterations=1, eps=1e-8):
  ...
  for _ in range(iterations):
    ...
    v *= np.rsqrt(tf.reduce_sum(tf.square(v)) + eps)
    u = tf.matmul(v, w_mat)
    sigma_inv = np.rsqrt(tf.reduce_sum(tf.square(u)) + eps)
    u *= sigma_inv

  with tf.control_dependencies([tf.Variable(u_var, u)]):
    return w * sigma_inv
...
def G_main(
  ...
  if not is_training or (
      dlatent_avg_beta is not None and not tflib.is_tf_expression(dlatent_avg_beta) and dlatent_avg_beta == 1):
    dlatent_avg_beta = None
  if not is_training or (
      style_mixing_prob is not None and not tflib.is_tf_expression(style_mixing_prob) and style_mixing_prob <= 0):
  ...
  if 'mapping' not in components:
    components.mapping = tflib.Network('G_mapping', func_name=globals()[mapping_func], dlatent_broadcast=num_layers,
                                       **kwargs)
  ...
  dlatent_avg = tf.compat.v1.get_variable('dlatent_avg', shape=[dlatent_size], initializer=tf.initializers.zeros(),
                                trainable=False)
  if dlatent_avg_beta is not None:
    with tf.compat.v1.variable_scope('DlatentAvg'):
      batch_avg = tf.reduce_mean(dlatents[:, 0], axis=0)
      update_op = tf.Variable(dlatent_avg, tflib.lerp(batch_avg, dlatent_avg, dlatent_avg_beta))
...
def G_mapping(
    ...
    # Output disentangled latent (W) as [minibatch, dlatent_size] or [minibatch, dlatent_broadcast, dlatent_size].
  ...
  for layer_idx in range(mapping_layers):
    with tf.compat.v1.variable_scope(f'Dense{layer_idx}'):
      ...
      x = apply_bias_act(dense_layer(x, fmaps=fmaps, lrmul=mapping_lrmul), act=mapping_nonlinearity,
                         lrmul=mapping_lrmul)
...
def G_synthesis(
    ...
    resample_kernel=None,  # Low-pass filter to apply when resampling activations, None = box filter.
  ...
  if use_noise:
    for layer_idx in range(num_layers - 1):
      ...
      noise_inputs.append(
        tf.compat.v1.get_variable(f'noise{layer_idx}', shape=shape, initializer=tf.initializers.random_normal(), trainable=False))
  ...
  def torgb(x, y, res):  # res = 2..resolution_log2
    with tf.compat.v1.variable_scope('ToRGB'):
      t = modulated_conv2d_layer(x, dlatents_in[:, res * 2 - 3], fmaps=num_channels, kernel=1, demodulate=False,
...
def D_main(
  ...
  def fromrgb(x, y, res):  # res = 2..resolution_log2
    with tf.compat.v1.variable_scope('FromRGB'):
      ...
      if pagan_bits is not None:
        with tf.compat.v1.variable_scope('PAGAN'):
          t += dense_layer(tf.cast(pagan_bits, t.dtype), fmaps=nf(res - 1), trainable=trainable)[:, :, np.newaxis,
               np.newaxis]
  ...
  def block(x, res):  # res = 2..resolution_log2
    ...
    with tf.compat.v1.variable_scope('Conv0'):
      trainable = is_next_layer_trainable()
      x = apply_bias_act(
        adrop(conv2d_layer(x, fmaps=nf(res - 1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)),
        act=act, clamp=conv_clamp, trainable=trainable)
    with tf.compat.v1.variable_scope('Conv1_down'):
      trainable = is_next_layer_trainable()
      x = apply_bias_act(adrop(
        conv2d_layer(x, fmaps=nf(res - 2), kernel=3, down=True, resample_kernel=resample_kernel, trainable=trainable,
                     use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
    if architecture == 'resnet':
      with tf.variable_scope('Skip'):
        trainable = is_next_layer_trainable()
        t = adrop(
          conv2d_layer(t, fmaps=nf(res - 2), kernel=1, down=True, resample_kernel=resample_kernel, trainable=trainable))
        x = (x + t) * (1 / np.sqrt(2))
    return x
  ...
  with tf.compat.v1.variable_scope('4x4'):
    ...
    with tf.compat.v1.variable_scope('Conv'):
      trainable = is_next_layer_trainable()
      x = apply_bias_act(
        adrop(conv2d_layer(x, fmaps=nf(1), kernel=3, trainable=trainable, use_spectral_norm=use_spectral_norm)),
        act=act, clamp=conv_clamp, trainable=trainable)
