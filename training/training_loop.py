...
def training_loop(
    ...
    G_args=None,  # Options for generator network.
    D_args=None,  # Options for discriminator network.
    G_opt_args=None,  # Options for generator optimizer.
    D_opt_args=None,  # Options for discriminator optimizer.
    loss_args=None,  # Options for loss function.
    train_dataset_args=None,  # Options for dataset to train with.
    metric_dataset_args=None,  # Options for dataset to evaluate metrics against.
    augment_args=None,  # Options for adaptive augmentations.
    metric_arg_list=None,  # Metrics to evaluate during training.
  ...
  if metric_arg_list is None:
    metric_arg_list = []
  if augment_args is None:
    augment_args = {}
  if metric_dataset_args is None:
    metric_dataset_args = {}
  if train_dataset_args is None:
    train_dataset_args = {}
  if loss_args is None:
    loss_args = {}
  if D_opt_args is None:
    D_opt_args = {}
  if G_opt_args is None:
    G_opt_args = {}
  if D_args is None:
    D_args = {}
  if G_args is None:
    ...
    with tf.name_scope(f'Train_gpu{gpu}'), tf.device(f'/gpu:{gpu}'):
      ...
      with tf.name_scope('DataFetch'):
        real_images_var = tf.Variable(name='images', trainable=False,
                                      initial_value=tf.zeros([minibatch_gpu] + training_set.shape))
        ...
        data_fetch_ops += [tf.Variable(real_images_var, real_images_write)]
        data_fetch_ops += [tf.Variable(real_labels_var, real_labels_write)]

      # Evaluate loss function and register gradients.
      fake_labels = training_set.get_random_labels_tf(minibatch_gpu)
      terms = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, aug=aug, fake_labels=fake_labels,
  ...
  Gs_epochs = tf.compat.v1.placeholder(tf.float32, name='Gs_epochs', shape=[])
  Gs_epochs_op = Gs.update_epochs(Gs_epochs)
  tflib.init_uninitialized_vars()
  # TODO find replacement for tf.contrib
  # with tf.device('/gpu:0'):
  ...
  while not done:
    ...
    if done or cur_tick < 0 or cur_nimg >= tick_start_nimg + kimg_per_tick * 1000:
      ...
      if image_snapshot_ticks is not None and (done or cur_tick % image_snapshot_ticks == 0):
        grid_fakes = Gs.run(grid_latents, grid_labels, is_validation=True, minibatch_size=minibatch_gpu)
        save_image_grid(grid_fakes, os.path.join(run_dir, f'fakes{cur_nimg // 1000:06d}.jpg'), drange=[-1, 1],
                        grid_size=grid_size)
