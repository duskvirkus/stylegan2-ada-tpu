...
class Projector:
  ...
  def set_network(self, Gs, dtype='float16'):
    ...
    while True:
      ...
      noise_init_ops.append(tf.Variable(v, tf.random.normal(tf.shape(v), dtype=tf.float32)))
      noise_mean = tf.reduce_mean(v)
      noise_std = tf.reduce_mean((v - noise_mean) ** 2) ** 0.5
      noise_normalize_ops.append(tf.Variable(v, (v - noise_mean) / noise_std))
    ...
    self._dlatents_var = tf.Variable(tf.zeros([self._minibatch_size] + list(self._dlatent_avg.shape[1:])),
                                     name='dlatents_var')
    ...
    self._dlatents_expr = tf.tile(self._dlatents_var + dlatents_noise,
    ...
    if sh[2] > 256:
      factor = sh[2] // 256
      proc_images_expr = tf.reduce_mean(
        tf.reshape(proc_images_expr, [-1, sh[1], sh[2] // factor, factor, sh[2] // factor, factor]), axis=[3, 5])
    ...
    if self._lpips is None:
      with dnnlib.util.open_url(
          'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada/pretrained/metrics/vgg16_zhang_perceptual.pkl') as f:
    ...
    for v in self._noise_vars:
      sz = v.shape[2]
      while True:
        reg_loss += tf.reduce_mean(v * tf.roll(v, shift=1, axis=3)) ** 2 + tf.reduce_mean(
  ...
  def start(self, target_images):
    ...
    if sh[2] > self._target_images_var.shape[2]:
      ...
      target_images = np.reshape(target_images, [-1, sh[1], sh[2] // factor, factor, sh[3] // factor, factor]).mean(
        (3, 5))
...
def main():
  ...
  parser.add_argument('--save-video', help='Save an mp4 video of optimization progress (default: true)',
                      type=_str_to_bool, default=True)
