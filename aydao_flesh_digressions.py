...
def generate_from_generator_adaptive(psi, radius_large, radius_small, step1, step2, video_length, seed, Gs):
  ...
  current_image = \
  Gs.run(current_latent, None, truncation_psi=psi, randomize_noise=False, output_transform=output_format)[0]
  output_frames = []
  ...
  while current_position_style < video_length:
    ...
    w = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if v.name == 'G_synthesis_1/4x4/Const/const:0'][0]
    ...
    tf.compat.v1.get_default_session().run(tf.Variable(w, v2))
    ...
    tf.compat.v1.get_default_session().run(tf.Variable(w, v1))
...
if __name__ == "__main__":
  ...
  parser.add_argument('--radius_large', help='The radius for the constant layer interpolation', default=300.0,
                      type=float)
  parser.add_argument('--radius_small', help='The radius for the latent space interpolation', default=40.0, type=float)
  parser.add_argument('--step1', help='The value of the step/increment for the constant layer interpolation',
                      default=0.005, type=float)
  parser.add_argument('--step2', help='The value of the step/increment for the latent space interpolation',
  ...
  parser.add_argument('--video_length',
                      help='The length of the video in terms of circular interpolation (recommended to keep at 1.0)',
