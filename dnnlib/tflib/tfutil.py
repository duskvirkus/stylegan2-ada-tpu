...
def log2(x: TfExpressionEx) -> TfExpression:
  ...
  with tf.name_scope("Log2"):
    return np.log(x) * np.float32(1.0 / np.log(2.0))
...
def _sanitize_tf_config(config_dict: dict = None) -> dict:
  ...
  cfg[
    "rnd.tf_random_seed"] = "auto"  # Random seed for TensorFlow. 'auto' = derive from NumPy random state. None = keep as is.
  cfg[
    "env.TF_CPP_MIN_LOG_LEVEL"] = "1"  # 0 = Print all available debug info from TensorFlow. 1 = Print warnings and errors, but disable debug info.
  cfg[
    "env.HDF5_USE_FILE_LOCKING"] = "FALSE"  # Disable HDF5 file locking to avoid concurrency issues with network shares.
  cfg[
    "graph_options.place_pruned_graph"] = True  # False = Check that all ops are available on the designated device. True = Skip the check for ops that are not used.
  cfg[
    "gpu_options.allow_growth"] = True  # False = Allocate all GPU memory at the beginning. True = Allocate only as much GPU memory as needed.
...
def convert_images_to_uint8(images, d_range=None, nchw_to_nhwc=False, shrink=1):
  ...
  scale = 255 / (d_range[1] - d_range[0])
  images = images * scale + (0.5 - d_range[0] * scale)
