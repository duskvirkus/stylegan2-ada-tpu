  ...
  def _get_device(self, device_name: str):
    ...
    with tfutil.absolute_name_scope(self.scope + "/Devices"), tf.device(device_name), tf.control_dependencies(None):
      if device_name not in self._shared_optimizers:
        ...
        self._shared_optimizers[device_name] = self.optimizer_class(name=optimizer_name,
                                                                    learning_rate=self.learning_rate,
                                                                    **self.optimizer_kwargs)
      device.optimizer = self._shared_optimizers[device_name]
      if self.use_loss_scaling:
        device.loss_scaling_var = tf.Variable(np.float32(self.loss_scaling_init), trainable=False,
    ...
    if self._report_mem_usage:
      ...
      try:
        with tf.name_scope(self.id + '_mem'), tf.device(device.name), tf.control_dependencies([loss]):
          # TODO find replacement for f.contrib.memory_stats.BytesInUse() in tf2
          deps.append(
            autosummary.autosummary(self.id + "/mem_usage_gb tf.contrib.memory_stats.BytesInUse() was removed in tensorflow 2")) #tf.contrib.memory_stats.BytesInUse() / 2 ** 30))
  ...
  def apply_updates(self, allow_no_op: bool = False) -> tf.Operation:
    ...
    if len(self._devices) > 1:
      with tfutil.absolute_name_scope(self.scope + "/Broadcast"), tf.device(None):
        ...
        elif tf.compat.v1.VERSION.startswith(
            "1.15."):  # TF 1.15 => NCCL ops are broken: https://github.com/tensorflow/tensorflow/issues/41539
          self._broadcast_fallback()
    ...
    for device_idx, device in enumerate(self._devices.values()):
      with tfutil.absolute_name_scope(self.scope + "/Apply%d" % device_idx), tf.device(device.name):
        ...
        if self.minibatch_multiplier is None:
          ...
          for var, grad in device.grad_clean.items():
            ...
            with tf.control_dependencies([acc_cur]):
              acc_inc_op = lambda: tf.Variable(acc_var, acc_cur)
              acc_reset_op = lambda: tf.Variable(acc_var, tf.zeros(var.shape))
              all_ops.append(tf.cond(acc_ok, acc_reset_op, acc_inc_op))

        # No overflow => apply gradients.
        all_ok = tf.reduce_all(tf.stack([acc_ok] + [tf.reduce_all(tf.compat.v1.is_finite(g)) for g in device.grad_acc.values()]))
        ...
        if device_idx == len(self._devices) - 1:
          ...
          all_ops.append(
            autosummary.autosummary(self.id + "/overflow_frequency", tf.where(all_ok, 0, 1), condition=acc_ok))
          if self.use_loss_scaling:
    ...
    if self.minibatch_multiplier is not None:
      tfutil.run([var.initializer for device in self._devices.values() for var in
                  list(device.grad_acc_vars.values()) + [device.grad_acc_count]])
...
class SimpleAdam:
  ...
  def apply_gradients(self, grads_and_vars):
    with tf.name_scope(self.name):
      ...
      update_ops += [tf.Variable(b1pow_var, b1pow_new), tf.Variable(b2pow_var, b2pow_new)]
      ...
      for grad, var in grads_and_vars:
        ...
        update_ops += [tf.Variable(m_var, m_new), tf.Variable(v_var, v_new), tf.compat.v1.assign_sub(var, var_delta)]
