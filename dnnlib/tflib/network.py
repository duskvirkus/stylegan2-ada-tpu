...
class Network:
  ...
  def __init__(self, name: str = None, func_name: Any = None, **static_kwargs):
    ...
    # Initialize fields.
    self._init_fields(name=(name or func_name), static_kwargs=static_kwargs, build_func=func, build_func_name=func_name,
                      build_module_src=module_src)

  def _init_fields(self, name: str, static_kwargs: dict, build_func: Callable, build_func_name: str,
                   build_module_src: str) -> None:
  ...
  def _init_graph(self) -> None:
    ...
    with tfutil.absolute_variable_scope(self.scope, reuse=False), tfutil.absolute_name_scope(self.scope), tf.device(
    ...
    self._own_vars = OrderedDict(
      (var.name[len(self.scope) + 1:].split(":")[0], var) for var in tf.compat.v1.global_variables(self.scope + "/"))
    ...
    if len(self._var_inits):
      tfutil.set_vars(
        {self._get_vars()[name]: value for name, value in self._var_inits.items() if name in self._get_vars()})
  ...
  def get_output_for(self, *in_expr: TfExpression, return_as_list: bool = False, **dynamic_kwargs) -> Union[
    TfExpression, List[TfExpression]]:
  ...
  def __setstate__(self, state: dict) -> None:
    ...
    # Initialize fields.
    self._init_fields(name=name, static_kwargs=static_kwargs, build_func=build_func, build_func_name=build_func_name,
                      build_module_src=build_module_src)
  ...
  def clone(self, name: str = None, **new_static_kwargs) -> "Network":
    ...
    net._init_fields(name=(name or self.name), static_kwargs=static_kwargs, build_func=self._build_func,
                     build_func_name=self._build_func_name, build_module_src=self._build_module_src)
  ...
  def setup_as_moving_average_of(self, src_net: "Network", beta: TfExpressionEx = 0.99,
  ...
  def run(self,
    ...
    key = dict(input_transform=input_transform, output_transform=output_transform, num_gpus=num_gpus,
               assume_frozen=assume_frozen, dynamic_kwargs=dynamic_kwargs)
    ...
    for mb_begin in range(0, num_items, minibatch_size):
      ...
      mb_in = [src[mb_begin: mb_end] if src is not None else np.zeros([mb_num] + shape[1:]) for src, shape in
               zip(in_arrays, self.input_shapes)]
  ...
  def list_layers(self) -> List[Tuple[str, TfExpression, List[TfExpression]]]:
    ...
    def recurse(scope, parent_ops, parent_vars, level):
      ...
      cur_vars = [(name, var) for name, var in parent_vars if
                  name.startswith(local_prefix) or name == local_prefix[:-1]]
      ...
      contains_direct_ops = any(
        "/" not in op.name[len(global_prefix):] and op.type not in ["Identity", "Cast", "Transpose"] for op in cur_ops)
      if (level == 0 or not contains_direct_ops) and (len(cur_ops) != 0 or len(cur_vars) != 0):
        visited = set()
        for rel_name in [op.name[len(global_prefix):] for op in cur_ops] + [name[len(local_prefix):] for name, _var in
                                                                            cur_vars]:
...
def _legacy_output_transform_func(*expr, out_mul=1.0, out_add=0.0, out_shrink=1, out_dtype=None):
  ...
  if out_shrink > 1:
    k_size = [1, 1, out_shrink, out_shrink]
    expr = [tf.nn.avg_pool(x, ksize=k_size, strides=k_size, padding="VALID", data_format="NCHW") for x in expr]
