...
def _find_compiler_bindir():
  hostx64_paths = sorted(
    glob.glob('C:/Program Files (x86)/Microsoft Visual Studio/*/Professional/VC/Tools/MSVC/*/bin/Hostx64/x64'),
    reverse=True)
  if hostx64_paths != []:
    return hostx64_paths[0]
  hostx64_paths = sorted(
    glob.glob('C:/Program Files (x86)/Microsoft Visual Studio/*/BuildTools/VC/Tools/MSVC/*/bin/Hostx64/x64'),
    reverse=True)
  if hostx64_paths != []:
    return hostx64_paths[0]
  hostx64_paths = sorted(
    glob.glob('C:/Program Files (x86)/Microsoft Visual Studio/*/Community/VC/Tools/MSVC/*/bin/Hostx64/x64'),
    reverse=True)
  if hostx64_paths != []:
    return hostx64_paths[0]
...
def get_plugin(cuda_file, extra_nvcc_options=None):
  if extra_nvcc_options is None:
    extra_nvcc_options = []
  ...
  try:
    ...
    md5.update(('tf.VERSION: ' + tf.compat.v1.VERSION).encode('utf-8') + b'\n')
