...
class HDF5Exporter:
  def __init__(self, h5_filename, resolution, channels, compress=False, expected_images=0, print_progress=True,
...
def create_from_images_raw(tfrecord_dir, image_dir, shuffle, resolution_log2=7, resize=None):
  ...
  with TFRecordExporter(tfrecord_dir, len(image_filenames), resolution_log2=resolution_log2) as tfr:
    ...
    for idx in range(order.size):
      ...
      #     error('Input images must have the same width and height')
      with tf.compat.v1.gfile.FastGFile(image_filenames[order[idx]], 'rb') as fid:
        try:
...
def create_from_image_folders_raw(tfrecord_dir, image_dir, shuffle, ignore_labels, resolution_log2=7):
  ...
  with TFRecordExporter(tfrecord_dir, len(images), resolution_log2=resolution_log2) as tfr:
    ...
    tfr.create_tfr_writer(img.shape)
    for idx in range(order.size):
      with tf.compat.v1.gfile.FastGFile(images[order[idx]], 'rb') as fid:
        try:
          tfr.add_image_raw(fid.read())
        except:
...
def hdf5_from_images(hdf5_filename, image_dir, compress):
  ...
  with HDF5Exporter(hdf5_filename, resolution=resolution, channels=channels, compress=compress,
                    expected_images=len(image_filenames)) as h5:
...
def execute_cmdline(argv):
  ...
  p.add_argument("tfrecord_dir", help="New dataset directory to be created")
  p.add_argument("image_dir", help="Directory containing the images")
  p.add_argument("--resize", help="resize to given power of 2 sized square images (default: None)", type=int,
                 default=None, required=False)
  p.add_argument("--shuffle", help="Randomize image order (default: 1)", type=int, default=1)
  p.add_argument("--resolution_log2", help="image width and height should be multiple of 2**res_log2 (default: 7)",
                 type=int, default=7)

  p = add_command('create_from_image_folders',
                  'Create dataset from a directory full of images. Use the folders to generate labels.',
                  'create_from_images datasets/mydataset myimagedir')
  p.add_argument('tfrecord_dir', help='New dataset directory to be created')
  p.add_argument('image_dir', help='Directory containing the images')
  p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)
  ...
  p = add_command('create_from_image_folders_raw',
                  'Create dataset from a directory full of images in raw format. Use the folders to generate labels.',
                  'create_from_images datasets/mydataset myimagedir')
  p.add_argument('tfrecord_dir', help='New dataset directory to be created')
  p.add_argument('image_dir', help='Directory containing the images')
  p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)
  ...
  p.add_argument('--resolution', help='Resize images to (resolution,resolution) (default: None = no resizing)',
  ...
  p.add_argument('--tfrecord_dir', help='New dataset directory to be created')
  p.add_argument('--num_train', help='Number of images to pick for the training set (default: None = all)', type=int,
                 default=None)
  p.add_argument('--num_validation', help='Number of images to pick for the validation set (default: 0 = no images)',
                 type=int, default=0)
  p.add_argument('--mirror', help='Number of images to pick for the training set (default: 0 = no mirroring)', type=int,
                 default=0)
  ...
  p.add_argument('--brecahad_dir', help='Source directory for BreCaHAD images.  Should contain .tif files.',
                 required=True)
