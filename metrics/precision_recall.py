...
class ManifoldEstimator():
  ...
  def evaluate(self, eval_features, return_realism=False, return_neighbors=False):
    ...
    for begin1 in range(0, num_eval_images, self.row_batch_size):
      ...
      realism_score[begin1:end1] = self.D[nearest_indices[begin1:end1], 0] / np.min(distance_batch[0:end1 - begin1, :],
                                                                                    axis=1)
...
def knn_precision_recall_features(ref_features, eval_features, feature_net, nhood_sizes,
  ...
  state.ref_manifold = ManifoldEstimator(distance_block, state.ref_features, row_batch_size, col_batch_size,
                                         nhood_sizes)
  state.eval_manifold = ManifoldEstimator(distance_block, state.eval_features, row_batch_size, col_batch_size,
                                          nhood_sizes)
  ...
  state.precision, state.realism_scores, state.nearest_neighbors = state.ref_manifold.evaluate(state.eval_features,
                                                                                               return_realism=True,
...
class PR(metric_base.MetricBase):
  ...
  def _evaluate(self, Gs, G_kwargs, num_gpus, **_kwargs):  # pylint: disable=arguments-differ
    ...
    state = knn_precision_recall_features(ref_features=feat_real, eval_features=feat_fake, feature_net=feature_net,
                                          nhood_sizes=[self.nhood_size], row_batch_size=self.row_batch_size,
